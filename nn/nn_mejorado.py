...
for umbral in x_list:
    df["predicted"] = df["Prediction"].apply(lambda x: 0 if x < umbral else 1)

    # calculate accuracy
    false_acceptance = df[(df["Label"] == 1) & (df["predicted"] == 0)].shape[0]
    false_rejection = df[(df["Label"] == 0) & (df["predicted"] == 1)].shape[0]
    false_acceptance_rate = false_acceptance / df[df["Label"] == 1].shape[0]
    false_rejection_rate = false_rejection / df[df["Label"] == 0].shape[0]
...
err = np.argmin(np.abs(np.array(fars) - np.array(frrs)))
err_x = x_list[err]
err = fars[err]
err = round(err, 4)

plt.clf()
plt.plot(x_list, fars, label='FAR')
plt.plot(x_list, frrs, label='FRR')
plt.xlabel('Umbral')
plt.ylabel('Error rate')
plt.title('CNN + 2 GRU')
plt.plot(err_x, err, 'ro', label='EER')
plt.text(err_x+0.02, err, f'EER: {err}')
plt.legend()
plt.savefig('gru.png')
